    ...
    class PrintFilteredStudentsCommand:Command
    {
        ...
        public override void Execute()
        {
            if (Data.Length == 5)
            {
                ...
                ParseParametersForFilterAndTake(command, quantity, course, filter);
            }
            else
            {
        ...
        private void ParseParametersForFilterAndTake(string command, string quantity, string course, string filter)
        {
            if (command == "take")
            {
                if (quantity == "all")
                ...
                else
                {
                    ...
                    if (hasParsed)
                    {
                        this.repository.FilterAndTake(course, filter, studentsToTake);
                    }
                    else
                    {
